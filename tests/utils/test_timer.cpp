...
TEST_F(TimerTest, PeriodicTaskExecutesMultipleTimes) {
  ...
  timer->addPeriodicTask(std::chrono::milliseconds(50),   // 初始延迟
...
TEST_F(TimerTest, HandlesMultipleDifferentTasks) {
  ...
  // 添加多个任务
  timer->addOnceTask(std::chrono::milliseconds(50),
                     [&task1_count]() { task1_count++; });
  timer->addOnceTask(std::chrono::milliseconds(100),
                     [&task2_count]() { task2_count++; });
  timer->addPeriodicTask(std::chrono::milliseconds(25),  // 初始延迟
