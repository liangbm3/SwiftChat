...
TEST(HttpRequestTest, ParseRequestWithHeaders) {
  ...
  EXPECT_EQ(req.getHeaderValue("Accept").value(),
            "application/json");  // 用大写 Accept 查询
...
TEST(HttpRequestTest, ParsePostRequestWithBody) {
  ...
  EXPECT_EQ(std::stoul(req.getHeaderValue("Content-Length").value().data()),
            body.length());
...
TEST(HttpRequestTest, HandleMalformedRequests) {
  ...
  // 2. 请求行不完整
  EXPECT_FALSE(http::HttpRequest::parse("GET / HTTP/1.1")
                   .has_value());  // 缺少结尾的\r\n
  EXPECT_FALSE(
      http::HttpRequest::parse("GET / \r\n\r\n").has_value());  // 缺少版本
