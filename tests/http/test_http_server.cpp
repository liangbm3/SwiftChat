...
TEST_F(HttpServerTest, RouteNotFound) {
  auto request_opt =
      http::HttpRequest::parse("GET /not-found HTTP/1.1\r\n\r\n");
  ASSERT_TRUE(request_opt.has_value());
...
TEST_F(HttpServerTest, MethodNotAllowed) {
  ...
  EXPECT_THAT(
      response_str,
      StartsWith(
          "HTTP/1.1 404 Not Found"));  // 修改期望值，因为没有匹配的路由会返回404
}

TEST_F(HttpServerTest, MiddlewareExecution) {
  // 添加一个中间件，它会给响应添加一个自定义Header
  server_.setMiddleware([](const http::HttpRequest& req,
                           const http::HttpServer::RequestHandler& next) {
...
TEST_F(StaticFileTest, ServesExistingFile) {
  auto request_opt =
      http::HttpRequest::parse("GET /index.html HTTP/1.1\r\n\r\n");
  ASSERT_TRUE(request_opt.has_value());
...
TEST_F(StaticFileTest, ReturnsNotFoundForMissingFile) {
  auto request_opt =
      http::HttpRequest::parse("GET /missing.css HTTP/1.1\r\n\r\n");
  ASSERT_TRUE(request_opt.has_value());
...
TEST_F(StaticFileTest, PreventsPathTraversal) {
  auto request_opt =
      http::HttpRequest::parse("GET /../secret.txt HTTP/1.1\r\n\r\n");
  ASSERT_TRUE(request_opt.has_value());
