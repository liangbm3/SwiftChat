...
TEST(HttpResponseTest, FluentInterfaceChaining) {
  ...
  EXPECT_THAT(resp_str,
              HasSubstr("Content-Length: 12\r\n"));  // "I'm a teapot" 的长度
  EXPECT_THAT(resp_str, EndsWith("\r\n\r\nI'm a teapot"));
}

TEST(HttpResponseTest, WithJsonBody) {
  nlohmann::json json_payload = {{"status", "success"},
                                 {"data", {1, "two", 3.0}}};
  ...
  auto resp =
      http::HttpResponse::Ok()
          ...
          .withHeader("cache-control",
                      "max-age=3600");  // key 是大小写不敏感的，但这里是标准
                                        // map，所以会区分
...
TEST(HttpResponseTest, CorrectContentLength) {
  // 1. 对于没有body的响应
  auto resp_no_body =
      http::HttpResponse::NoContent();  // 假设我们添加一个204工厂
  // 或者用现有的
  ...
  EXPECT_THAT(
      resp_with_body.toString(),
      HasSubstr("Content-Length: " + std::to_string(body.length()) + "\r\n"));
}