...
struct ServerConfig {
  ...
  unsigned int mysql_port = 4406;
  std::string mysql_database = "swiftchat";
...
void showHelp(const char* program_name) {
  ...
  std::cout << "  " << program_name
            << " --db-path /var/lib/swiftchat/chat.db\n";
}
...
ServerConfig parseCommandLine(int argc, char* argv[]) {
  ...
  int c;
  while ((c = getopt_long(argc, argv, "h:w:H:P:D:U:W:s:l:?v", long_options,
                          nullptr)) != -1) {
    switch (c) {
      case 'h':
      ...
      case 'H':
        ...
        break;
      case 'P':
        config.mysql_port = std::atoi(optarg);
        break;
      case 'D':
        config.mysql_database = optarg;
        break;
      case 'U':
        config.mysql_username = optarg;
        break;
      case 'W':
        config.mysql_password = optarg;
        break;
      case 's':
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (!std::getenv("JWT_SECRET")) {
      ...
      LOG_WARN << "JWT_SECRET environment variable set to default value - "
                  "请在生产环境中设置安全密钥";
    }

    // 初始化数据库管理器
    MySQLConfig mysql_config;
    mysql_config.host = config.mysql_host;
    mysql_config.port = config.mysql_port;
    mysql_config.database = config.mysql_database;
    mysql_config.username = config.mysql_username;
    mysql_config.password = config.mysql_password;

    DatabaseManager db_manager(mysql_config);
    LOG_INFO << "数据库管理器已初始化: " << config.mysql_host << ":"
    ...
    std::cout << "HTTP 服务器: http://localhost:" << config.http_port
              << std::endl;
    std::cout << "WebSocket 服务器: ws://localhost:" << config.ws_port
              << std::endl;
    std::cout << "访问 http://localhost:" << config.http_port << " 开始使用"
              << std::endl;
