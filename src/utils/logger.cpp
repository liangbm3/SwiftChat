...
Logger::LogStream::LogStream(LogLevel level, const char* file,
    ...
    : level_(level),
      ...
      function_(function),
      line_(line),
      should_log_(level >= Logger::getGlobalLevel()) {
  if (should_log_) {
    ...
    stream_ << Color::CYAN << "[" << buffer << "." << std::setfill('0')
            << std::setw(6) << now_ms.count() << "] " << getLevelColor(level_)
            << Color::BOLD << "[" << getLevelStr(level_) << "] "
            << Color::MAGENTA << "[" << std::this_thread::get_id() << "] "
            << Color::BLUE << "[" << file_ << ":" << line_ << "] "
            << Color::CYAN << "[" << function_ << "] " << getLevelColor(level_);
  }
}
...
bool Logger::isFileLoggingEnabled() { return file_logging_enabled_; }

// Logger 静态工厂方法
Logger::LogStream Logger::Debug(const char* file, const char* function,
                                int line) {
  return LogStream(LogLevel::DEBUG, file, function, line);
}

Logger::LogStream Logger::Info(const char* file, const char* function,
                               int line) {
  return LogStream(LogLevel::INFO, file, function, line);
}

Logger::LogStream Logger::Warn(const char* file, const char* function,
                               int line) {
  return LogStream(LogLevel::WARN, file, function, line);
}

Logger::LogStream Logger::Error(const char* file, const char* function,
                                int line) {
  return LogStream(LogLevel::ERROR, file, function, line);
}

Logger::LogStream Logger::Fatal(const char* file, const char* function,
                                int line) {
