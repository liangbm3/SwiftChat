...
AuthService::AuthService(DatabaseManager &db_manager)
...
void AuthService::registerRoutes(http::HttpServer &server) {
  http::HttpServer::Route register_route{
      ...
      .handler = [this](const http::HttpRequest &request)
          -> http::HttpResponse { return registerUser(request); },
  ...
  http::HttpServer::Route login_route{
      ...
      .method = "POST",
      .handler = [this](const http::HttpRequest &request)
          -> http::HttpResponse { return loginUser(request); },
  ...
  http::HttpServer::Route logout_route{
      ...
      .method = "POST",
      .handler = [this](const http::HttpRequest &request)
          -> http::HttpResponse { return logoutUser(request); },
...
http::HttpResponse AuthService::registerUser(const http::HttpRequest &request) {
  try {
    ...
    LOG_INFO << "User does not exist, proceeding with registration for: "
             << username;
...
http::HttpResponse AuthService::createAndSignToken(const User &user,
  ...
  auto token =
      jwt::create()
          ...
          .set_expires_at(std::chrono::system_clock::now() +
                          std::chrono::hours(1))  //过期时间，1小时后
          .set_subject(
              user.getId())  //设置主题
                             //(sub)，这是标准声明，通常用来存放用户的唯一标识符
          .set_payload_claim(
              "username",
              jwt::claim(user.getUsername()))  //自定义声明，存放用户名
          .sign(jwt::algorithm::hs256{
              secret_key});  //使用哈希算法HS256和密钥进行签名

  //构造HTTP响应
  std::string success_message =
      is_registration ? "User registered successfully" : "Login successful";
  ...
  // 根据操作类型返回适当的状态码
  if (is_registration) {
    return http::HttpResponse::Created().withJsonBody(
        response_json);  // 201 Created for registration
  } else {
    return http::HttpResponse::Ok().withJsonBody(
        response_json);  // 200 OK for login
  }
}
