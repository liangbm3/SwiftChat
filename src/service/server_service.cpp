...
void ServerService::registerRoutes(http::HttpServer& server) {
  ...
  http::HttpServer::Route echo_get_route{
      "/api/v1/echo", "GET",
      [this](const http::HttpRequest& req) { return this->handleEchoGet(req); },
      false  // 不需要认证
  };
...
http::HttpResponse ServerService::handleHealthCheck(
...
http::HttpResponse ServerService::handleServerInfo(
    const http::HttpRequest& req) {
  json response = {{"success", true},
  ...
  return http::HttpResponse::Ok().withBody(response.dump(), "application/json");
}
...
http::HttpResponse ServerService::handleEchoGet(const http::HttpRequest& req) {
  ...
  std::string user_agent = user_agent_opt.has_value()
                               ? std::string(user_agent_opt.value())
                               : "Unknown";
...
http::HttpResponse ServerService::handleProtected(
    const http::HttpRequest& req) {
  json response = {{"success", true},
