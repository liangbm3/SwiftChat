...
MessageService::MessageService(DatabaseManager &db_manager)
...
std::optional<std::string> MessageService::getUserIdFromRequest(
    const http::HttpRequest &request) {
  return JwtUtils::getUserIdFromRequest(request);
}

// GET /api/v1/messages?room_id=...&limit=...&before=...
http::HttpResponse MessageService::getMessages(
    const http::HttpRequest &request) {
  //确认用户已经登录
  ...
  if (auto limit_opt = request.getQueryParam("limit")) {
    try {
      ...
      LOG_ERROR << "Invalid limit parameter: "
                << std::string(limit_opt->data(), limit_opt->size())
                << ". Using default value of 50.";
      limit = 50;
    }
  }
  ...
  if (std::find_if(menbers.begin(), menbers.end(), [&](const json &member) {
        return member["id"] ==
               user_id;  // 注意：这里应该是 "id" 而不是 "user_id"
      }) == menbers.end()) {
  ...
  try {
    ...
    std::transform(messages.begin(), messages.end(),
                   ...
                   [](const auto &message) { return message.toJson(); });
    ...
    LOG_ERROR << "Failed to retrieve messages for room " << room_id << ": "
              << e.what();
    json error_response = {{"success", false},
