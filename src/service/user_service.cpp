...
UserService::UserService(DatabaseManager &db_manager)
    : db_manager_(db_manager) {}

void UserService::registerRoutes(http::HttpServer &server) {
  // 注册获取当前用户信息的路由
  server.addHandler({.path = "/api/v1/users/me",
                     .method = "GET",
                     .handler =
  ...
  // 注册获取所有用户列表的路由
  server.addHandler({.path = "/api/v1/users",
                     .method = "GET",
                     .handler =
                         [this](const http::HttpRequest &request) {
  ...
  // 注册获取指定用户信息的路由
  server.addHandler({.path = "/api/v1/users/{userId}",
                     .method = "GET",
                     .handler =
                         [this](const http::HttpRequest &request) {
  ...
  // 注册获取用户状态的路由
  server.addHandler({.path = "/api/v1/users/{userId}/status",
                     .method = "GET",
                     .handler =
                         [this](const http::HttpRequest &request) {
                           return handleGetUserStatus(request);
                         },
                     .use_auth_middleware = true});
}

std::optional<std::string> UserService::getUserIdFromRequest(
...
http::HttpResponse UserService::handleGetAllUsers(
    const http::HttpRequest &request) {
  auto user_id_opt = getUserIdFromRequest(request);
  if (!user_id_opt) {
    LOG_ERROR << "Failed to get user ID from request.";
  ...
  try {
    ...
    if (start_index < total_count) {
      users.assign(all_users.begin() + start_index,
                   all_users.begin() + end_index);
    }
...
http::HttpResponse UserService::handleGetUserById(
    const http::HttpRequest &request) {
  ...
  try {
    ...
    if (!user_opt) {
      LOG_ERROR << "User with ID '" << target_user_id
    ...
    LOG_ERROR << "Failed to retrieve user information for ID '"
              << target_user_id << "': " << e.what();
    json error_response = {{"success", false},
                           {"message", "Failed to retrieve user information"},
                           {"error", e.what()}};
    return http::HttpResponse::InternalError().withJsonBody(error_response);
...
http::HttpResponse UserService::handleGetUserStatus(
    const http::HttpRequest &request) {
  auto user_id_opt = getUserIdFromRequest(request);
  if (!user_id_opt) {
    LOG_ERROR << "Failed to get user ID from request.";
  ...
  try {
    ...
    json status_data = {{"user_id", target_user_id},
    ...
    LOG_ERROR << "Failed to retrieve user status for ID '" << target_user_id
              << "': " << e.what();
    json error_response = {{"success", false},
