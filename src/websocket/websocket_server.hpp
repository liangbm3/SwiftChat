...
class WebSocketServer {
 ...
 private:
  ...
  // 消息处理辅助方法
  void handle_authenticated_message(connection_hdl hdl,
                                    const std::string &user_id,
                                    const nlohmann::json &message);
  void handle_join_room(connection_hdl hdl, const std::string &user_id,
                        const nlohmann::json &message);
  void handle_leave_room(connection_hdl hdl, const std::string &user_id,
                         const nlohmann::json &message);
  void handle_chat_message(connection_hdl hdl, const std::string &user_id,
                           const nlohmann::json &message);
  ...
  // 用户和连接管理
  std::unordered_map<std::string, connection_hdl>
      user_connections_;  // 用户ID到连接句柄的映射
  std::unordered_map<connection_hdl, std::string, ConnectionHdlHash,
                     ConnectionHdlEqual>
      connection_users_;  // 连接句柄到用户ID的映射

  // 房间管理
  std::unordered_map<std::string, std::set<std::string>>
      room_members_;  // 房间ID到用户ID集合的映射
  std::unordered_map<std::string, std::string>
      user_current_room_;  // 用户ID到当前房间ID的映射
};