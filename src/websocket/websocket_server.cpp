  ...
  try {
    ...
    // 关闭所有现有连接
    {
      ...
      for (const auto &pair : connection_users_) {
        websocketpp::lib::error_code ec;
        server_.close(pair.first, websocketpp::close::status::going_away,
                      "Server shutdown", ec);
        if (ec) {
...
void WebSocketServer::on_message(connection_hdl hdl,
  ...
  if (!is_authenticated)  // 处理未认证的连接，期望收到的认证消息
  {
    try {
      ...
      if (json_msg.value("type", "") == "auth") {
        ...
        if (!verified_user_id) {
          ...
          server_.send(hdl, error_response.dump(),
        ...
        {  // 进入临界区
          ...
          if (old_connection_it != user_connections_.end()) {
            LOG_INFO << "User " << verified_id
                     << " already has a connection. Closing old connection.";
            // 获取旧连接句柄
            // 向旧连接发送通知
            ...
            try {
              server_.close(old_connection_it->second,
                            websocketpp::close::status::policy_violation,
                            reason.dump());
            } catch (const std::exception &e) {
              LOG_ERROR << "Error closing old connection for user "
                        << verified_id << ": " << e.what();
            }
        ...
        LOG_INFO << "WebSocket connection authenticated for user: "
                 << verified_id;
        json response = {
            {"success", true},
      ...
      server_.close(hdl, websocketpp::close::status::invalid_payload,
                    "Invalid JSON format");
    } catch (const std::exception &e) {
      ...
      server_.send(hdl, error_response.dump(),
                   websocketpp::frame::opcode::text);
      server_.close(hdl, websocketpp::close::status::internal_endpoint_error,
    ...
    try {
      ...
      LOG_ERROR << "JSON parsing error from user " << user_id << ": "
                << e.what();
      send_error(hdl, "Invalid JSON format");
    } catch (const std::exception &e) {
      LOG_ERROR << "Message handling error from user " << user_id << ": "
                << e.what();
...
void WebSocketServer::handle_authenticated_message(connection_hdl hdl,
                                                   const std::string &user_id,
                                                   const json &message) {
  std::string msg_type = message.value("type", "");

  LOG_INFO << "Received message type '" << msg_type
           << "' from user: " << user_id;
...
void WebSocketServer::handle_join_room(connection_hdl hdl,
                                       const std::string &user_id,
                                       const json &message) {
  try {
    ...
    std::lock_guard<std::mutex> lock(
        connection_mutex_);  // 访问共享数据，进入临界区
    // 检查用户是否在房间中
    auto current_room_it = user_current_room_.find(user_id);
    if (current_room_it != user_current_room_.end() &&
        current_room_it->second == room_id) {
      LOG_WARN << "User " << user_id << " tried to join room " << room_id
               << " but is already in it.";
    ...
    if (current_room_it != user_current_room_.end()) {
      ...
      // 通知原房间内其他用户该用户已离开
      json leave_notification = {{"success", true},
                                 {"message", "User left room"},
                                 {"data",
                                  {{"type", "user_left"},
                                   {"user_id", user_id},
                                   {"username", username},
                                   {"room_id", old_room_id}}}};
      broadcast_to_room(old_room_id, leave_notification.dump(),
                        user_id);  // 排除自己
    ...
    LOG_ERROR << "Exception joining room for user " << user_id << ": "
              << e.what();
    send_error(hdl, "Failed to join room");
  }
}

void WebSocketServer::handle_leave_room(connection_hdl hdl,
                                        const std::string &user_id,
                                        const json &message) {
  std::lock_guard<std::mutex> lock(connection_mutex_);
  auto current_room_it = user_current_room_.find(user_id);
  ...
  json response = {
      ...
      {"data",
       {{"type", "room_left"}, {"room_id", room_id}, {"user_id", user_id}}}};
  server_.send(hdl, response.dump(), websocketpp::frame::opcode::text);
}

void WebSocketServer::handle_chat_message(connection_hdl hdl,
                                          const std::string &user_id,
                                          const json &message) {
  try {
    ...
    try {
      ...
      LOG_INFO << "Message saved to database from user " << user_id
               << " in room " << room_id;
    } catch (const std::exception &e) {
    ...
    LOG_ERROR << "Error processing chat message from user " << user_id << ": "
              << e.what();
    send_error(hdl, "Missing required field: content");
  }
}

void WebSocketServer::join_room(const std::string &user_id,
                                const std::string &room_id) {
  room_members_[room_id].insert(user_id);
  user_current_room_[user_id] = room_id;
}

void WebSocketServer::leave_room(const std::string &user_id,
                                 const std::string &room_id) {
...
void WebSocketServer::send_error(connection_hdl hdl,
...
void WebSocketServer::broadcast_to_room(const std::string &room_id,
...
void WebSocketServer::broadcast_to_room(const std::string &room_id,
  ...
  std::vector<connection_hdl> connections_to_send;  // 需要发送的连接
  {
    ...
    auto room_it = room_members_.find(room_id);
    if (room_it == room_members_.end()) {
      LOG_WARN << "Attempted to broadcast to non-existent or empty room: "
               << room_id;
      return;
    }
  ...
  LOG_INFO << "Broadcasting message to " << connections_to_send.size()
           << " users in room: " << room_id;
