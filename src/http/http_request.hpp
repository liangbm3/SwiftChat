...
class HttpRequest {
 public:
  static std::optional<HttpRequest> parse(
      const std::string&
          raw_request);  // 成功返回一个HttpRequest对象，失败返回std::nullopt
  ...
  std::optional<std::string_view> getHeaderValue(
      const std::string& key) const;  // 获取指定请求头的值

  bool hasQueryParam(const std::string& key) const;  // 检查是否有指定的查询参数
  std::optional<std::string_view> getQueryParam(
      const std::string& key) const;  // 获取指定查询参数的值

  bool hasCookie(const std::string& key) const;  // 检查是否有指定的Cookie
  std::optional<std::string_view> getCookieValue(
      const std::string& key) const;  // 获取指定Cookie的值
  ...
  std::optional<std::string_view> getPathParam(
      const std::string& key) const;  // 获取指定路径参数的值
  const std::unordered_map<std::string, std::string>& getPathParams() const {
    return path_params_;
  }
  void setPathParams(
 ...
 private:
  ...
  std::string urlDecode(
      const std::string& encoded_str);  //对url编码的字符串进行解码
  std::string method_;                  // 请求方法，如 GET、POST 等
  std::string path_;                    // 请求路径
  std::string version_;                 // HTTP 版本，如 HTTP/1.1
  std::string body_;                    // 请求体内容
  std::unordered_map<std::string, std::string, CaseInsensitiveHasher,
                     CaseInsensitiveEqual>
      headers_;  // 请求头映射，忽略大小写
