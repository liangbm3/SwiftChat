...
void Router::setStaticDirectory(const std::string &dir) { static_dir_ = dir; }
...
bool Router::matchPath(const std::string &pattern, const std::string &path,
  ...
  for (size_t i = 0; i < patternSegments.size(); ++i) {
    ...
    // 检查是否为参数段（以{开头并以}结尾）
    if (patternSeg.length() > 2 && patternSeg.front() == '{' &&
        patternSeg.back() == '}') {
      // 提取参数名（去掉{}）
...
HttpResponse Router::serveStaticFile(const std::string &path) {
  ...
  std::string full_path =
      static_dir_ + (safe_path == "/" ? "/index.html" : safe_path);
  ...
  if (file_size > MAX_FILE_SIZE) {
    LOG_WARN << "File too large: " << full_path << " (" << file_size
             << " bytes)";
