...
std::optional<HttpRequest> HttpRequest::parse(const std::string &raw_request) {
  ...
  if (head_end_pos == std::string::npos) {
    LOG_ERROR
        << "Malformed request: Missing header/body separator (\\r\\n\\r\\n).";
    return std::nullopt;
  }
  ...
  if (!(request_line_ss >> request.method_ >> request.path_ >>
        request.version_)) {
  ...
  while (std::getline(head_ss, line)) {
    ...
    if (line.empty()) continue;  // 忽略头部中的空行
  ...
  if (request.hasHeader("Content-Length")) {
    try {
      size_t content_length =
          std::stoul(request.getHeaderValue("Content-Length").value().data());
      ...
      if (raw_request.length() < body_start_pos + content_length) {
        LOG_ERROR << "Incomplete request body. Expected " << content_length
                  << " bytes, but only "
                  << (raw_request.length() - body_start_pos) << " available.";
        return std::nullopt;
      }
...
std::optional<std::string_view> HttpRequest::getHeaderValue(
...
std::optional<std::string_view> HttpRequest::getQueryParam(
    const std::string &key) const {
...
std::optional<std::string_view> HttpRequest::getCookieValue(
    const std::string &key) const {
...
std::string HttpRequest::urlDecode(const std::string &encoded_str) {
  ...
  for (size_t i = 0; i < encoded_str.length(); ++i) {
    if (encoded_str[i] == '%' && i + 2 < encoded_str.length()) {
      if (std::isxdigit(encoded_str[i + 1]) &&
          std::isxdigit(encoded_str[i + 2])) {
        try {
...
std::optional<std::string_view> HttpRequest::getPathParam(
    const std::string &key) const {
