...
Room Room::fromJson(const json &j) {
  ...
  if (j.contains("id") && j["id"].is_number_integer()) room.id_ = j["id"];

  if (j.contains("name") && j["name"].is_string()) room.name_ = j["name"];

  if (j.contains("description") && j["description"].is_string())
    room.description_ = j["description"];

  if (j.contains("creator_id") && j["creator_id"].is_number_integer())
